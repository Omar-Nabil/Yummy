...
$(function() {
    ...
    getAreas().then((data) => {
        ...
        for(let i=0; i<data.length; i++) {
            homeDiv.innerHTML += `
                <div class="main-cat text-center text-white p-2" onclick="areaDetails('${data[i].strArea}')">
                    <div class="first img-fluid mb-3">
                        <i class="fa-solid fa-house-laptop fa-2xl"></i>
                    </div>
                    <div class="text-center">
                        <h2 class="mx-1 fw-bold display-6">${data[i].strArea}</h2>
                    </div>
                </div>
            `;
        }
...
async function getareaByName(areaName) {
...
function areaDetails(areaName) {
    ...
    getareaByName(areaName).then((data)=> {
        $('.lds-spinner').fadeOut(1000, 
            () => {
                $('#loading').animate({top:'-100%'}, 1000,
                    () => {
