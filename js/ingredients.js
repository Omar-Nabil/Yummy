...
$(function() {
    ...
    getingrs().then((data) => {
        ...
        for(let i=0; i<data.length; i++) {
            homeDiv.innerHTML += `
                <div class="main-cat text-center text-white p-2" onclick="ingrDetails('${data[i].strIngredient}')">
                    <div class="first img-fluid mb-3">
                        <i class="fa-solid fa-drumstick-bite fa-2xl"></i>
                    </div>
                    <div class="text-center">
                        <h2 class=" fw-bold display-6">${data[i].strIngredient.split(" ").slice(0,1).join(" ")}</h2>
                        <p>${data[i].strDescription ? data[i].strDescription.split(" ").slice(0,13).join(" "): 'Unknown'}</p>
                    </div>
                </div>
            `;
        }
...
async function getintgByName(ingrName) {
    let response = await fetch(`https://www.themealdb.com/api/json/v1/1/filter.php?i=${ingrName}`);
    let result = await response.json();
    
    return result.meals;
}
function ingrDetails(ingrName) {
    $('.lds-spinner').css('display', 'block');
    $('#loading').css('top', '0');
    $('body').css('overflow', 'hidden');
    getintgByName(ingrName).then((data)=> {
        $('.lds-spinner').fadeOut(1000, 
            () => {
                $('#loading').animate({top:'-100%'}, 1000,
                    () => {
        ...
        let productes = document.getElementById('producte');

        remove.classList.add('d-none');
        productes.classList.remove('d-none');
...
function productDetails(productId) {
    ...
    getMealById(productId).then((data)=> {
        ...
        product.classList.add('text-white');
